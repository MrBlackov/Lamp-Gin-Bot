...
async def cmd_handler(message: Message, state: FSMContext, **kwargs):
    msg, markup = await ItemService(message.from_user.id, state, message).add.to_name(message)
...
@exept
async def cmd_handler(message: Message, state: FSMContext, **kwargs):
    msg, markup = await ItemService(message.from_user.id, state, message).add.to_create_item()
...
async def callback_handler(callback: CallbackQuery, callback_data: NewItemACtionCall, state: FSMContext, **kwargs):
    msg, markup = await ItemService(callback.from_user.id, state, callback.message).add.to_name(callback.message)
...
async def cmd_handler(message: Message, state: FSMContext, **kwargs):
    ...
    msg, markup = await ItemService(message.from_user.id, state, message).add.to_emodzi(message.text, message)
...
async def cmd_handler(message: Message, state: FSMContext, **kwargs):
    fsm = ItemFSM(state, 'new')
    msg0 = await fsm.get_value('msg')
    msg, markup = await ItemService(message.from_user.id, state, message).add.to_menu(message.text)
    msg2 = await message.answer(msg, reply_markup=markup)
    await fsm.update_data(msg=msg2)
    await fsm.set_state()
    await msg0.delete()
...
async def callback_handler(callback: CallbackQuery, callback_data: NewItemBackCall, state: FSMContext, **kwargs):
    ...
    msg, markup = await ItemService(callback.from_user.id, state, callback.message).add.menu()
    await callback.message.edit_text(msg, reply_markup=markup)
...
async def cmd_handler(message: Message, state: FSMContext, **kwargs):
    fsm = ItemFSM(state, 'new')
    msg0 = await fsm.get_value('msg')
    msg, markup = await ItemService(message.from_user.id, state, message).add.redact(message)
    msg2 = await message.answer(msg, reply_markup=markup)
    await fsm.update_data(msg=msg2)
    await fsm.set_state()
    await msg0.delete()
...
@call_exept()
async def callback_handler(callback: CallbackQuery, callback_data: NewItemACtionCall, state: FSMContext, **kwargs):
    msg, markup = await ItemService(callback.from_user.id, state, callback.message).add.to_delete_nbt()
    await callback.message.edit_text(msg, reply_markup=markup)
...
@call_exept()
async def callback_handler(callback: CallbackQuery, callback_data: NewItemACtionCall, state: FSMContext, **kwargs):
    msg, markup = await ItemService(callback.from_user.id, state, callback.message).add.delete_nbt()
    await callback.message.edit_text(msg, reply_markup=markup)
...
async def callback_handler(callback: CallbackQuery, callback_data: NewItemACtionCall, state: FSMContext, **kwargs):
    await ItemFSM(state, 'new').set_state()
    msg, markup = await ItemService(callback.from_user.id, state, callback.message).add.nbt()
    await callback.message.edit_text(msg, reply_markup=markup)
...
async def callback_handler(callback: CallbackQuery, callback_data: NewItemACtionCall, state: FSMContext, **kwargs):
    await ItemFSM(state, 'new').set_state()
    msg, markup = await ItemService(callback.from_user.id, state, callback.message).add.to_action_tag()
    await callback.message.edit_text(msg, reply_markup=markup)
...
async def cmd_handler(message: Message, state: FSMContext, **kwargs):
    fsm = ItemFSM(state, 'new')
    msg0 = await fsm.get_value('msg')
    msg, markup = await ItemService(message.from_user.id, state, message).add.add_action_tag(message.text)
    msg2 = await message.answer(msg, reply_markup=markup)
    await fsm.update_data(msg=msg2)
    await fsm.set_state()
    await msg0.delete()
...
async def callback_handler(callback: CallbackQuery, callback_data: NewItemSketchDeleteActionTagCall, state: FSMContext, **kwargs):
    msg, markup = await ItemService(callback.from_user.id, state, callback.message).add.to_delete_action_tag(callback_data.tag)
    await callback.message.edit_text(msg, reply_markup=markup)
...
@call_exept()
async def callback_handler(callback: CallbackQuery, callback_data: NewItemSketchDeleteActionTagCall, state: FSMContext, **kwargs):
    msg, markup = await ItemService(callback.from_user.id, state, callback.message).add.delete_action_tag(callback_data.tag)
    await callback.message.edit_text(msg, reply_markup=markup)
...
@call_exept()
async def callback_handler(callback: CallbackQuery, callback_data: NewItemACtionCall, state: FSMContext, **kwargs):
    msg, markup = await ItemService(callback.from_user.id, state, callback.message).add.to_send()
    await callback.message.edit_text(msg, reply_markup=markup)
...
@call_exept()
async def callback_handler(callback: CallbackQuery, callback_data: NewItemACtionCall, state: FSMContext, **kwargs):
    msg, markup = await ItemService(callback.from_user.id, state, callback.message).add.create()
    await callback.message.edit_text(msg, reply_markup=markup)
...
async def callback_handler(callback: CallbackQuery, callback_data: NewItemAdminACtionCall, state: FSMContext, **kwargs):
    msg, markup = await ItemService(callback.from_user.id, state, callback.message).change.info(callback_data.sketch_id)
...
@call_exept(False)
async def callback_handler(callback: CallbackQuery, callback_data: NewItemAdminACtionCall, state: FSMContext, **kwargs):
    msg, markup = await ItemService(callback.from_user.id, state, callback.message).add.create_after_moderating(callback_data.sketch_id, callback_data.to_create)
    await callback.message.edit_text(msg, reply_markup=markup)
