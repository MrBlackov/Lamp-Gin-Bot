...
@new_char_router.message(NewCharState.part_name, F.content_type == 'text', F.text.not_contains('/'))
@log.decor(arg=True)
@exept
async def cmd_handler(message: Message, state: FSMContext, **kwargs):
...
@new_char_router.message(NewCharState.description, F.content_type == 'text', F.text.not_contains('/'))
@log.decor(arg=True)
@exept
async def cmd_handler(message: Message, state: FSMContext, **kwargs):
    fsm = CharFSM(state, 'new')
