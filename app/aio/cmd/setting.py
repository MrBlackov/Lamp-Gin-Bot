...
async def cmd_handler(message: Message, state: FSMContext, **kwargs):
    msg, markup = await SettingService(message.from_user.id, state).get_user_setting('user')
...
async def callback_to_new_item_faq(callback: CallbackQuery, callback_data: SettingBackCall | MenuCall, state: FSMContext, **kwargs):
    msg, markup = await SettingService(callback.from_user.id, state).get_user_setting('user')
...
@call_exept()
async def callback_to_new_item_faq(callback: CallbackQuery, callback_data: SettingBackCall | MenuCall, state: FSMContext, **kwargs):
    msg, markup = await SettingService(callback.from_user.id, state).get_user_setting('char')
    await callback.message.edit_text(msg, reply_markup=markup)
...
async def callback_to_new_item_faq(callback: CallbackQuery, callback_data: SettingRedactCall, state: FSMContext, **kwargs):
    msg, markup = await SettingService(callback.from_user.id, state).redact_setting(callback_data.tag, callback_data.type)
    await callback.message.edit_text(msg, reply_markup=markup)
...
async def callback_to_new_item_faq(callback: CallbackQuery, callback_data: SettingActionCall, state: FSMContext, **kwargs):
    msg, markup = await SettingService(callback.from_user.id, state).to_default(callback_data.type)
    await callback.message.edit_text(msg, reply_markup=markup)
...
@call_exept()
async def callback_to_new_item_faq(callback: CallbackQuery, callback_data: SettingActionCall, state: FSMContext, **kwargs):
    msg, markup = await SettingService(callback.from_user.id, state).to_insert_json(callback_data.type, callback.message)
    await callback.message.edit_text(msg, reply_markup=markup)
...
async def cmd_handler(message: Message, state: FSMContext, **kwargs):
    ...
    msg, markup = await SettingService(message.from_user.id, state).insert_json(message.text)
