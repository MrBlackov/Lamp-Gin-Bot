...
async def cmd_handler(message: Message, state: FSMContext, **kwargs):
    msg, markup = await SocialService(message.from_user.id, state).get_friends()
...
async def callback_handler(callback: CallbackQuery, callback_data: SocialBackCall | MenuCall, state: FSMContext, **kwargs):
    msg, markup = await SocialService(callback.from_user.id, state).get_friends()
...
async def callback_handler(callback: CallbackQuery, callback_data: SocialFriendCall, state: FSMContext, **kwargs):
    msg, markup = await SocialService(callback.from_user.id, state).friend(callback_data.user_id)
    await callback.message.edit_text(msg, reply_markup=markup)
...
@call_exept()
async def callback_handler(callback: CallbackQuery, callback_data: SocialFriendCall, state: FSMContext, **kwargs):
    msg, markup = await SocialService(callback.from_user.id, state).delete_friend(callback_data.user_id)
    await callback.message.edit_text(msg, reply_markup=markup)
...
async def callback_handler(callback: CallbackQuery, callback_data: SocialActionCall, state: FSMContext, **kwargs):
    msg, markup = await SocialService(callback.from_user.id, state).to_send_request(callback.message)
    await callback.message.edit_text(msg, reply_markup=markup)
...
async def cmd_handler(message: Message, state: FSMContext, **kwargs):
    ...
    msg, markup = await SocialService(message.from_user.id, state).send_request(message.text)
...
async def callback_handler(callback: CallbackQuery, callback_data: SocialRequestCall, state: FSMContext, **kwargs):
    msg, markup = await SocialService(callback.from_user.id, state).answer_request(callback_data.user_id, callback_data.status)
    await callback.message.edit_text(msg, reply_markup=markup)
