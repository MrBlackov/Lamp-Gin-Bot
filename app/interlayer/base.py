...
class BaseLayer:
    ...
    async def checking_freedom(self, tag: str | None = None):
        ...
        for action_state in action_states:
            if action_state.tag == ActionTags.sleep and tag != ActionTags.wake_up and tag != ActionTags.stop:
                raise SleepError(f'💤 Char sleep, user(id={self.user.id}, tg_id={self.user.tg_id})')
            elif action_state.is_block_freedom and tag != ActionTags.stop and tag != ActionTags.wake_up:
